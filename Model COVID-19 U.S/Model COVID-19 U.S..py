	...
	def run(self):
		...
		for key in ('confirmed','deaths'):
			if key=='confirmed':
				...
				Data=12
			Dataset=pd.read_csv(os.path.join('Dataset','time_series_covid_19_'+key+'_US.csv'))
			Coordinates=Dataset.iloc[:,8:10]
			Data=Dataset.iloc[:,Data:]
			...
			for i in Data.columns:
				...
				IMAGE=cv2.imread('template.png')
				cv2.putText(IMAGE,'Updating',(300,200),cv2.FONT_HERSHEY_COMPLEX,4,(0,0,0),3)
				cv2.putText(IMAGE,'Finished processing Image: '+str(counter+start)+'/'+str(last*2),(400,300),cv2.FONT_HERSHEY_COMPLEX_SMALL,1,(0,0,0),1)
				cv2.imwrite(os.path.join('Images','Updating.png'),IMAGE)
				'''
					while cv2.waitKey(1)==-1:
					cv2.imshow('Model Corona',IMAGE)
				'''
				self.Signal.emit(counter+start,last*2)
...
class MyWindow(QMainWindow):
	def __init__(self):
		...
		self.setWindowTitle("Model COVID-19 U.S.")
		self.build()
	def Disable(self):
